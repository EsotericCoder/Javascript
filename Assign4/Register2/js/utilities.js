...
/*3: Change the addTooltipHandlers() function so that it also applies appropriate
event listeners to the form’s label. HINT: Start by changing the function
so that it takes the element’s id value as its argument, then either give each
label an id value of id + ‘Label’, or use the previousSibling reference,
as in the showTooltip() and hideTooltip() functions.

5: Change the addTooltipHandlers() function so that it uses addEventListener(),
attachEvent(), and the corollary remove event functions instead
of the DOM Level 0 approach.
*/
