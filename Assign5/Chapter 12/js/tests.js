...
var myTests = function() {
	...
	//Bullet 5: test that $() returns Undefined when no argument is give:
	function testNoElement(){
		jsUnity.assertions.assertUndefined(U.$());
	}
	//Bullet 5: test that setText() returns Undefined when no argument is give:
	function testNoSetText(){
		jsUnity.assertions.assertUndefined(U.setText());
	}
	//Bullet 5: test that $() returns NotTypeOf when invalid type is given :
	function testInvalidTypeElement(){
		jsUnity.assertions.assertNotTypeOf('string' , U.$('doesNotExist'));
	}
	//Bullet 5: test that $() returns NotTypeOf when invalid type is given :
	function testInvalidTypeSetText(){
		jsUnity.assertions.assertNotTypeOf('string', U.setText('wrong argument'));
	}
