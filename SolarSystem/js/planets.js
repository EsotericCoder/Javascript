...
//This function accepts the index number for info list, title name, and pic location
function displayPlanet(num, name, pic){
'use strict';
	
	//pic location placed in HTML
	U.$('pic').innerHTML = pic;

	//textContent and innerText to output and title
    if (output.textContent != undefined) {
        output.textContent = planetInfo[num];
        title.textContent = name;

    } else {
        output.innerText = planetInfo[num];
        title.textContent = name;
    }
...
'use strict';
	...
	//Use second menu selection to determine information, pic, and name.
	switch(selection){
		case "Andromeda Galaxy":
			image = '<img src= images/andromeda.jpg>';
			text = galaxyInfo[1];
			break;
		case "M82 Galaxy":
			image = '<img src= images/m82.jpg>';
			text = galaxyInfo[2];
			break;
		case "Milky Way":
			image = '<img src= images/milky.jpg>';
			text = galaxyInfo[3];
			break;
		case "NGC 5866":
			image = '<img src= images/ngc.jpg>';
			text = galaxyInfo[4];
			break;
		case "Omega Centauri":
			image = '<img src= images/omega.jpg>';
			text = galaxyInfo[5];
			break;
		case "Whirlpool Galaxy":
			image = '<img src= images/whirlpool.jpg>';
			text = galaxyInfo[6];
			break;
	}
...
'use strict';
	...
	var planet = U.$('planet');
	//initialize
	var selection = parseInt(planet.value);
	var options = null;
	var name = "";
	var image = "";

	
	// Empty the second menu when selecting planets:
	if(U.$('opt')){
		var galaxiesMenu = U.$('opt');
		galaxiesMenu.parentNode.removeChild(galaxiesMenu);
	}
	
	//use selection displayPlanet or add second menu
	switch(selection){
		case 1:
			name = "Earth";
            image = "<img src= images/earth.jpg>";
            displayPlanet(1, name, image);
            break;
		case 2:
			name = "Jupiter";
            image = "<img src= images/jupiter.jpg>";
            displayPlanet(2, name, image);
            break;
		case 3:
			name = "Mars";
            image = "<img src= images/mars.jpg>";
            displayPlanet(3, name, image);
            break;
		case 4:
			name = "Mercury";
            image = "<img src= images/mercury.jpg>";
            displayPlanet(4, name, image);
            break;
		case 5:
			name = "Neptune";
            image = "<img src= images/neptune.jpg>";
            displayPlanet(5, name, image);
            break;
		case 6:
			name = "Saturn";
            image = "<img src= images/saturn.jpg>";
            displayPlanet(6, name, image);
            break;
		case 7:
			name = "Uranus";
            image = "<img src= images/uranus.jpg>";
            displayPlanet(7, name, image);
            break;
		case 8:
			name = "Venus";
            image = "<img src= images/venus.jpg>";
            displayPlanet(8, name, image);
            break;
		case 9:
			//Create a second menu div for formatting
			var secondMenu = document.createElement('div');
			secondMenu.id = 'secondMenu';
			planet.parentNode.appendChild(secondMenu);
			//Create a second menu select feature for the dropdown
			var galaxiesMenu = document.createElement('select');
			galaxiesMenu.id = 'opt';
			galaxiesMenu.disabled = false;
			planet.parentNode.appendChild(galaxiesMenu); 
			var galaxies = U.$('opt');
			// options list
			...
			for (var i = 0, count = options.length; i < count; i++) {
		    	...
		    	galaxies.appendChild(opt);	
		    }
		    //Add event listener to displayGalaxy on change
		    U.addEvent(U.$('opt'), 'change', displayGalaxy);
			break;
	}
...
'use strict';
	...
	//Add event listener to updateMenu on change	 
	U.addEvent(U.$('planet'), 'change', updateMenu);
